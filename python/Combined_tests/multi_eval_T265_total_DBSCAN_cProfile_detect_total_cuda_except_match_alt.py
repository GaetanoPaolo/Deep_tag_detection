...
kp_temp_orb_2_cuda, des_temp_orb_2_cuda = orb.detectAndComputeAsync(logo_temp_2_cuda,None)
kp_temp_orb_5_cuda, des_temp_orb_5_cuda = orb.detectAndComputeAsync(logo_temp_5_cuda,None)
kp_temp_orb_10_cuda, des_temp_orb_10_cuda = orb.detectAndComputeAsync(logo_temp_10_cuda,None)
...
def pose_est(kp_temp_orb_2, des_temp_orb_2,
    ...
    if rel_pos_c[2] < 3.0:
            ...
            est_orb_2, inlier_am_2= det.detect_match(K,kp_temp_orb_2,des_temp_orb_2,logo_temp_2_shape,kp_target,des_target,bf_HAMMING,src_gray,logo_temp_2)
            est_orb_5, inlier_am_5= det.detect_match(K,kp_temp_orb_5,des_temp_orb_5,logo_temp_5_shape,kp_target,des_target,bf_HAMMING,src_gray,logo_temp_5)
            est_orb_10,inlier_am_10 = det.detect_match(K,kp_temp_orb_10,des_temp_orb_10,logo_temp_10_shape,kp_target,des_target,bf_HAMMING,src_gray,logo_temp_10)

            est_orb_lst = [est_orb_2,est_orb_5,est_orb_10]
            inlier_am_lst = [inlier_am_2,inlier_am_5,inlier_am_10]
            idx = 0
            if sum(inlier_am_lst) == 0:
                trans = zerarr
            else:
                idx = inlier_am_lst.index(max(inlier_am_lst))
                if inlier_am_lst[idx] < 20:
                    trans = zerarr
                else:
...
for ep in range(1,2):
    ...
    for observed_pos in range(160,161):
        ...
        #             "cuda_total_except_match_pose_est_above_3m_196_FAST_20.dat")
        start = time.time()
...
#         output_hdf5_path = output_dir + '/T265_alt_DBSCAN_profile_total_cv_cuda_except_match_FAST_20' + '.hdf5'
...
#             )
#         hdf5_file.close()
# dump(current_dir,hdf5_data,'T265_alt_DBSCAN_profile_total_cv_cuda_except_match_FAST_20')
