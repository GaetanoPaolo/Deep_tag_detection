...
for ep in range(1,len(base_items)):
    ...
    for observed_pos in range(0,set_size[0]):
        ...
        if rel_pos_c[2] < 3.0:
            ...
            rot = 0
            pos_temp_world = dw.world_coord(np.array(corn_temp),src_gray.shape,rot)
            dist_coeffs = np.zeros((4,1))
            (suc,rot,trans,inliers) = cv.solvePnPRansac(pos_temp_world, np.array(ordered_pts), K, dist_coeffs, flags=cv.SOLVEPNP_ITERATIVE, iterationsCount=2000, reprojectionError=8.0)
...
def dump(output_dir,hdf5_data,ep):
        ...
        output_hdf5_path = output_dir + '/psi_800res_alt_rot_rec_3' + '.hdf5'
        ...
        hdf5_file.close()
dump(current_dir,hdf5_data,'psi_800res_alt_rot_optimal_rec_3')