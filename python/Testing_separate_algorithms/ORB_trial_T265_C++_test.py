...
print(base_items)
dset1 = f.get('1')
K = np.array(dset1.get('K'),dtype=np.float32).reshape((3,3))
...
rel_pos_d = np.matmul(R_w_d,np.array(rel_pos_w, dtype = np.float32))
...
if rel_pos_c[2] > 3:
    ...
    print('Kp array lengths')
    print(len(pos_temp))
    print(len(pos_temp))
    #apply solvepnp to estimate translation: https://learnopencv.com/head-pose-estimation-using-opencv-and-dlib/
    pos_temp_world = dw.world_coord(np.array(pos_temp),logo_temp,rot)
    #dist_coeffs = np.array([-0.014216,0.060412,-0.054711,0.011151],dtype=np.float64)
    dist_coeffs = np.zeros((4,1),dtype=np.float32)
    #(suc,rot,trans,inliers) = cv.solvePnPRansac(pos_temp_world, np.array(pos_target), K, dist_coeffs, flags=cv.SOLVEPNP_ITERATIVE, iterationsCount=200, reprojectionError=2)
    ...
    trans = np.array(solve_res[0:3])
    inlier_amount = solve_res[3]
